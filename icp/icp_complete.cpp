...
int main(int argc, const char *argv[])
{
	...
	const int ret = validateArgs(argc, argv, isTransfoSaved, configFile, outputBaseFile, initTranslation, initRotation);
	...
	PM::TransformationParameters T = icp(initializedSource, target);
	float matchRatio = icp.errorMinimizer->getWeightedPointUsedRatio();
	cout << "match ratio: " <<  matchRatio << endl;

	// Transform data to express it in target
	PointCloud source_out(initializedSource);
	...
	source_out.save(outputBaseFile + "source_out.vtk");
	if(isTransfoSaved)
	{
